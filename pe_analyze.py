#!/usr/bin/env python3
"""Analyze PE structure of VCDS.exeL to find file offset for RVA 0x140076ff0."""
import struct
import sys

EXE_PATH = r"D:\VCDS Test\ARM64\Installation\VCDS\VCDS.exeL"
TARGET_RVA = 0x76FF0  # RVA = 0x140076FF0 - ImageBase 0x140000000
...
with open(EXE_PATH, 'rb') as f:
    ...
    if target_section:
        ...
        # Read the bytes at that location
        f.seek(offset)
        original_bytes = f.read(16)
        print(f'Original bytes at offset 0x{offset:X}: {original_bytes.hex(" ")}')
        
        # Check file offset 0x1FA31B
        f.seek(0x1FA31B)
        version_byte = f.read(1)
        print(f'Version byte at file offset 0x1FA31B: 0x{version_byte.hex()}')
    else:
